 ...
 */
function computeScore(words, avgWps, hashtagCount, hasCta) {
    let score = 0;
    // Word count
    if (words > 0 && words <= 280)
        score += 30;
    else if (words <= 500)
        score += 22;
    else if (words <= 2200)
 ...
 */
function assessPlatforms(words, hashtagCount, hasCta, emojiCount) {
    // Twitter / X: ≤280 chars (rough: ≤60 words), 1–3 hashtags, CTAs ok
    const twitter = words <= 60
        ? {
            fit: "good",
    ...
    const linkedin = words >= 80 && words <= 1300
        ? {
            fit: "good",
    ...
    const instagram = words <= 150 && hashtagCount >= 5
        ? {
            fit: "good",
    ...
    const facebook = words >= 40 && words <= 500 && hasCta
        ? {
            fit: "good",
